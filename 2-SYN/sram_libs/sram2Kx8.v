...
//       
//       Creation Date:  Wed Dec 30 14:26:33 2020
//       Version: 	r0p0
//
...
`ifdef POWER_PINS
module sram2Kx8 (VDDCE, VDDPE, VSSE, Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, RET1N);
`else
module sram2Kx8 (Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, RET1N);
`endif
 ...
 wire  CLK_;
  ...
  assign RET1N_ = RET1N;
   ...
   `ifdef ARM_FAULT_MODELING
     sram2Kx8_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
  ...
  reg NOT_D0, NOT_EMA2, NOT_EMA1, NOT_EMA0, NOT_EMAW1, NOT_EMAW0, NOT_EMAS, NOT_RET1N;
 ...
 wire  CLK_;
  ...
  buf B33(EMAW_[0], EMAW[0]);
  buf B34(EMAW_[1], EMAW[1]);
  buf B35(EMAS_, EMAS);
  buf B36(RET1N_, RET1N);
...
`endif
`timescale 1ns/1ps
module sram2Kx8_error_injection (Q_out, Q_in, CLK, A, CEN, WEN);
   output [7:0] Q_out;
   input [7:0] Q_in;
   input CLK;
   input [10:0] A;
